...
$pluginname		 = get_string('pluginname', 'repository_mergefiles');

$PAGE->set_url('/repository/mergefiles/index.php', array('id' => $course->id));
$PAGE->set_title($course->shortname.' | '.$pluginname);
$PAGE->set_heading($course->fullname.' | '.$pluginname);
$PAGE->navbar->add($pluginname);

echo $OUTPUT->header();
$heading = "Course files";	// PDFs only
echo $OUTPUT->heading($heading);
echo "(Note: Files in pdf format only)";
echo "<br><br>";
...
if ($data = $mform->get_data()) {
	if (!empty($data->save)) {
		...
		$backuppath .= "/";
		$mergedpdfdoc = $backuppath.uniqid('mergedfile_').".pdf";

		$stampedfile->copy_content_to($mergedpdfdoc);
